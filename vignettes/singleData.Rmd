...
\begin{equation*}
  \mathbf{y} = \sum_{g \in \mathcal{G}} \phi( \mathbf{X}_{g}) \theta_g + \boldsymbol\varepsilon, \qquad \boldsymbol\varepsilon \sim \mathcal{N}(\mathbf{0}_n, \mathbf{I}_n \sigma^2),
\end{equation*}
where
  ...
  - $\boldsymbol\theta$ is a vector with $K$ coefficients (one per group).
