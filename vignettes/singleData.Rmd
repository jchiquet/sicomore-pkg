...
This vignette illustrates how SIComORe can be used for the analysis of a single data set with correlated predictors related to a phenotype. 
...
\begin{equation*}
  \mathbf{y} = \sum_{g \in \mathcal{G}} \phi( \mathbf{X}_{g}) \boldsymbol\theta_g + \boldsymbol\varepsilon, \qquad \boldsymbol\varepsilon \sim \mathcal{N}(\mathbf{0}_n, \mathbf{I}_n \sigma^2),
\end{equation*}
where
  ...
  - $\boldsymbol\theta$ is a vector withg $K$ coefficients (one per group).
  
SIComORe includes the function `getHierLevel` to estimate both the underlying group structure $\mathcal{G}$ and the vector of regression $\boldsymbol\theta$ from the data $(\mathbf{y},\mathbf{X})$. We detail its use on a simulated example bellow.
...
The matrix of empirical correlation of $X$ clearly show that the group structure is well embedded in the data:
...
The vector of regression parameters $\theta$ is sparse, chosen such that the first two group are predictive:
...
SIComORe also includes three alternative methods for prediction of phenotype with grouped correlated structure, on top of the one defined in @sicomore. MLGL (see @grimonprez_PhD, @mlgl) and the variant $\rho$-SIComORe in the spirit of @park. We try the 3 available options for simultaneously finding and selecting the groups of variables related to the phenotype:

```{r sim res}
out_rho      <- getHierLevel(X, y, hierarchy, choice="lambda.1se", selection="rho-sicomore", mc.cores=NULL)
out_mlgl     <- getHierLevel(X, y, hierarchy, choice="lambda.1se", selection="mlgl"        , mc.cores=NULL)
out_sicomore <- getHierLevel(X, y, hierarchy, choice="lambda.1se", selection="sicomore"    , mc.cores=NULL)
all_res <- rbind(
